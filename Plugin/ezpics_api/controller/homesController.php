...
function createLayerVariable($input)
{
    ...
    if(!empty($session->read('infoUser')) && $isRequestPost){
        ...
        if(!empty($product) && $product->type=='user_series' && !empty($dataSend['nameVariable'])){
            ...
            $new->name = 'Layer '.$idlayer;
            $new->products_id = $dataSend['idproduct'];
            $new->content = json_encode(getLayer($idlayer,$dataSend['type'],@$dataSend['banner'],$dataSend['width'], $dataSend['height'], $dataSend['text'], $dataSend['nameVariable'], $dataSend['variableLabel'], 'Arial', '#000', '10vw',$dataSend['typeShowTextVariable']));
            $new->sort = $idlayer;
            $new->created_at = date('Y-m-d H:i:s');
