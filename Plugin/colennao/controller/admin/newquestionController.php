<?php 
function listnewquetionsyoga($input)

{
    ...
    $modelQuestions = $controller->loadModel('newsquestions');
...
function addnewquetionyoga($input)
...
function deletenewQuestion($input){
	global $controller;
	$modelQuestions = $controller->loadModel('newsquestions');
	if(!empty($_GET['id'])){
		$data = $modelQuestions->find()->where(['id'=>(int) $_GET['id']])->first();
...
function listnewquestionkarate($input)
...
function addnewquetionkarate($input)
...
function deletenewQuestionkarate($input){
	global $controller;
	$modelQuestions = $controller->loadModel('newsquestions');
	if(!empty($_GET['id'])){
		$data = $modelQuestions->find()->where(['id'=>(int) $_GET['id']])->first();
