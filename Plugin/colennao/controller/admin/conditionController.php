...
function listcondition($input){
    ...
    $datacondition = $modeltbcondition->find()->where()->order(['id' => 'ASC'])->all();
    ...
    $listData = $modeltbcondition->find()->limit($limit)->page($page)->where()->order($order)->all()->toList();
 
    // phân trang
    $totalData = $modeltbcondition->find()->where()->all()->toList();
...
function addcondition($input){
    ...
    if ($isRequestPost) {
        ...
        if (!empty($dataSend['id_groupfile']) && !empty($dataSend['id_question'])) {
            ...
            $type = $dataSend['type']; // Lấy giá trị type từ dữ liệu gửi lên
            foreach ($dataSend['id_question'] as $questionId) {
                ...
                if ($existingData) {
                    ...
                    $existingData->type = $type; // Cập nhật thêm cột type
                    $modeltbcondition->save($existingData);
                } else {
                    // Thêm mới dữ liệu nếu chưa tồn tại
                    $data = $modeltbcondition->newEmptyEntity();
    ...
    $dataWorkout = $userpeople->find()->where()->order(['id' => 'asc'])->all()->toList();
    $dataquestion = $modelQuestions->find()->where()->order(['id' => 'asc'])->all()->toList();
