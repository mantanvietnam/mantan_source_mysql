     ...
     if(!empty($session->read('infoUser'))){
       ...
       //   $question ='   Please answer me in Tiếng Việt language and also respond in Tiếng Việt language . Dựa vào thông sản phẩm, dịch vụ '.$dataSend['topic'].', hãy xác nhận bằng cách viết"';
           ...
           /* if(!empty($dataSend['topic'])){
            ...
            $reply = '<h1>Tạo landing page đỉnh cao</h1>'.$reply_ai['result'];
            ...
            $chat = array();
             ...
             if(!empty($dataSend['type'])){
                if($dataSend['type']==1){
                    ...
                    $title = '<h1>Section 3</h1>';
                    $question = "Hãy đưa ra các ưu điểm và lợi ích độc đáo của sản phẩm  1 cách thật sự sống động để khách hàng như cảm thấy chân thực những lợi ích này.. Please answer me in Tiếng Việt language and also respond in Tiếng Việt language.";
                }elseif($dataSend['type']==5){
                    $title = '<h1>Section 4</h1>';
