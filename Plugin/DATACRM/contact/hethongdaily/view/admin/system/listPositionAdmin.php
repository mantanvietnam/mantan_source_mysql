...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4">Hệ thống</h4>
                        ...
                        if(!empty($listData)){
                          foreach ($listData as $item) {
                            echo '<tr>
                                    ...
                                    <td>
                                      <span class="text-danger">'.$item->name.'</span> ('.$item->description.'%)
                                      <br/>
                                      '.number_format($item->number_member).' đại lý
                                    </td>
                                    <td align="center">
                                      <a class="dropdown-item" href="javascript:void(0);" onclick="editData('.$item->id.', \''.$item->name.'\', \''.$item->parent.'\', \''.$item->keyword.'\', \''.$item->description.'\' );">
                          ...
                          echo '<tr>
                                  <td colspan="3" align="center">Chưa có hệ thống</td>
                                </tr>';
                        }
                      ?>
          ...
          <div class="card mb-6">
            ...
            <div class="card-body">
              <?= $this->Form->create(); ?>
                ...
                <div class="mb-3">
                  <label class="form-label" for="basic-default-phone">Hệ thống</label>
                  <select class="form-select color-dropdown" name="parent" id="parent">
                    <?php 
                    if(!empty($listSystemAdmin)){
                      foreach ($listSystemAdmin as $key => $value) {
                ...
                <div class="mb-3">
                  <label class="form-label" for="basic-default-phone">Tên chức danh</label>
  ...
  <script type="text/javascript">
    function editData(id, name, parent, keyword, description){
    ...
    function deleteCategory(id){
      ...
      if(check){
        $.ajax({
          ...
          .done(function( msg ) {
            window.location = '/plugins/admin/hethongdaily-view-admin-system-listPositionAdmin';
          })
          .fail(function() {
            window.location = '/plugins/admin/hethongdaily-view-admin-system-listPositionAdmin';
          });
