<?php 
function listCategoryGroupcamera($input){
    global $isRequestPost;
    global $modelCategories;
    global $metaTitleMantan;

    $metaTitleMantan = 'Danh sách danh mục camera';
    ...
    if ($isRequestPost) {
        ...
        $infoCategory->type = 'category_groupcamera';
        ...
        $number = 0;
        do{
            $conditions = array('slug'=>$slugNew,'type'=>'category_groupcamera');
...
function listCategoryDeportmentcamera($input){
    global $isRequestPost;
    global $modelCategories;
    global $metaTitleMantan;

    $metaTitleMantan = 'Danh sách chức năng camera';
    ...
    if ($isRequestPost) {
        ...
        $infoCategory->keyword = str_replace(array('"', "'"), '’', $dataSend['keyword']);
        $infoCategory->description = str_replace(array('"', "'"), '’', $dataSend['description']);
        $infoCategory->type = 'Category_deportment';
        ...
        $number = 0;
        do{
            $conditions = array('slug'=>$slugNew,'type'=>'Category_deportment');
            $listData = $modelCategories->find()->where($conditions)->order(['id' => 'DESC'])->all()->toList();
