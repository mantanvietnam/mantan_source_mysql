...
function updateBookCarAPI($input){
 	global $isRequestPost;
	global $controller;
	global $session;
	...
	if($isRequestPost){
		...
		if(!empty($infoUser)){
			...
			if(!empty($book)){
				...
				$return= array('code'=>1,
								'book_code'=> $book->id,
								'mess' => 'bạn sửa thông tin đặt xe thành công',
							);
			}else{
				$return = array('code'=>3, 'mess'=>'idBook không tồn tại');
			}
		}else{
			$return = array('code'=>2, 'mess'=>'Bạn chưa đăng nhập');
		}
...
function historyUserBookAPI($input){
	...
	if($isRequestPost){
		$dataSend = $input['request']->getData();
		$infoUser = $modelMember->find()->where(array('token'=>$dataSend['token'], 'type'=>0))->first();
...
function historyDriverBookAPI($input){
	global $isRequestPost;
	global $controller;
	global $session;
	...
	if($isRequestPost){
		...
		if(!empty($infoUser)){
			$conditions = array('driver_id'=>$infoUser->id);
			if(isset($dataSend['status'])){
				$conditions['status'] = $dataSend['status'];
			}
