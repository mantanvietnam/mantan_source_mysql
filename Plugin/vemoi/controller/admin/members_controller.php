        ...
        if(!empty($dataSend['name']) && !empty($dataSend['phone'])){
        	...
        	if(empty($checkPhone) || (!empty($_GET['id']) && $_GET['id']==$checkPhone->id) ){
        		if(empty($dataSend['avatar'])){
                    $dataSend['avatar'] = $urlHomes.'/plugins/vemoi/view/home/assets/img/avatar-default-crm.png';
                }
...
function deleteMemberAdmin($input){
    ...
    return $controller->redirect('/plugins/admin/vemoi-view-admin-member-listMemberAdmin');
}
?>