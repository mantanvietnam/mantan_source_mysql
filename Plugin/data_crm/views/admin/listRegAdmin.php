    ...
    <div class="card mb-4">
      ...
      <div class="card-body">
        <div class="row gx-3 gy-2 align-items-center">
          ...
          <div class="col-md-2">
            <label class="form-label">&nbsp;</label>
            <a href="/plugins/admin/data_crm-views-admin-updateCodeAdmin" class="btn btn-danger d-block">Nâng cấp code</a>
          </div>
        </div>
      </div>
    </div>
          ...
          <?php 
            if(!empty($listData)){
              foreach ($listData as $item) {
                if($item->status == 'done'){
                  $link = '<a href="https://'.$item->system_slug.'.icham.vn" target="_blank">'.$item->system_slug.'</a>';
                }else{
                  $link = $item->system_slug;
                }
                echo '<tr>
                        <td>'.$item->id.'</td>
                        <td>
