...
<<<<<<< HEAD
    ...
    <div class="max-w-4xl">
        <?php  
        $price_total = 0; // Khởi tạo tổng tiền
        $total = 0;
=======
      <div class="step-content p-2" data-step="1">
        <div class="max-w-4xl">
...
>>>>>>> 59318f8831ad034e7ef2a13df396ebf14f754f0b
        ...
        if (!empty($list_product)) {
            foreach ($list_product as $key => $value) {
                // Kiểm tra và xử lý giá trị của số lượng và giá sản phẩm
                $numberOrder = isset($value->numberOrder) ? intval($value->numberOrder) : 0;
                $price = isset($value->price) ? floatval($value->price) : 0;

<<<<<<< HEAD
                $link = '/product/' . htmlspecialchars($value->slug) . '.html';
                $price_old = !empty($value->price_old) 
                    ? '<del>' . number_format($value->price_old) . '₫</del>' 
                    : '';

                // Tính giá sản phẩm
                $price_buy = $price * $numberOrder;
                $price_total += $price_buy; // Cộng dồn vào tổng tiền
                $total = $price_total; // Cập nhật tổng tiền
=======
...
>>>>>>> 59318f8831ad034e7ef2a13df396ebf14f754f0b
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex-1 product-info">
                        ...
                        <p class="text-gray-500 product-quantity-label">Số lượng: ' . $numberOrder . '</p>
                        <button
                            class="flex items-center mt-4 text-red-500 delete-product"
                            onclick="window.location.href=\'/deleteProductCart/?id_product=' . htmlspecialchars($value->id) . '\'"
                        >
                            <i class="mr-1 fas fa-trash-alt"></i> Xóa sản phẩm
                        </button>
                    </div>
                    <div class="text-right product-price">
                        ...
                        <div class="flex items-center mt-2 quantity-control">
                            ...
                            <span class="mx-2 quantity-display">' . $numberOrder . '</span>
                            <button
                                class="flex items-center justify-center w-8 h-8 border rounded-full btn-increment"
                            >
                                +
