				...
				<option value="">Danh mục sản phẩm</option>
				 <?php if(!empty($listCate)) {
                    foreach ($listCate as $key => $value) { ?>
                        <option value="<?php echo $value['MerchandiseGroup']['id'] ?>"><?php echo $value['MerchandiseGroup']['name'] ?></option>
                    <?php
                    }
                } ?>
			</select>
				...
				foreach ($tmpVariable['listData'] as $key => $value) { ?>
					<div class="col-6 col-sm-6 col-md-4 col-lg-3 item-product">
						<a href="<?php echo $urlHomes.'product/'.@$value['Merchandise']['urlSlug'].'.html' ?>">
		  				...
		  				<center class="add-cart"><button class="add-cart-button" onclick="addToCart(this, '<?php echo @$value['Merchandise']['id'] ?>')">Thêm vào giỏ hàng <span class="messAdd">Đã thêm vào giỏ</span></button></center>
					</div>
				<?php
				}
			} ?>
