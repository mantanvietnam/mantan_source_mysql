            ...
            <select name="category" id=""  onchange="loadCate(this)">
                ...
                <?php if(!empty($listCate)) {
                    foreach ($listCate as $key => $value) { ?>
                        <option <?php echo $tmpVariable['category']['MerchandiseGroup']['urlSlug']==$value['MerchandiseGroup']['urlSlug']?'selected':''; ?> value="<?php echo @$value['MerchandiseGroup']['urlSlug'] ?>"><?php echo $value['MerchandiseGroup']['name'] ?></option>
                    <?php
                    }
                } ?>
            </select>
            ...
            <?php if(!empty($tmpVariable['listData'])) {
                foreach ($tmpVariable['listData'] as $key => $value) { ?>
                    <div class="col-6 col-sm-6 col-md-4 col-lg-3 item-product">
                        ...
                        <center class="add-cart"><button class="add-cart-button" data-sku="<?php echo @$value['Merchandise']['code'] ?>" onclick="addToCart('<?php echo @$value['Merchandise']['id'] ?>')">Thêm vào giỏ hàng</button></center>
                    </div>
                <?php
                }
            } ?>
