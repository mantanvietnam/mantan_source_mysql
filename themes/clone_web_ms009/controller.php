...
function registerEvent($input)
{
    ...
    if($isRequestPost){
        ...
        if(!empty($dataSend['name']) && !empty($dataSend['phone']) && !empty($dataSend['avatar'])){
            $dataSend['phone'] = trim(str_replace(array(' ','.','-'), '', $dataSend['phone']));
            $dataSend['phone'] = str_replace('+84','0',$dataSend['phone']);

            $conditions = ['phone'=>$dataSend['phone']];
            $checkPhone = $modelCustomers->find()->where($conditions)->first();

            $avatar = [];
            if(isset($_FILES['avatar']) && empty($_FILES['avatar']["error"])){
                $avatar = uploadImage('customer', 'avatar', 'avatar_'.$checkPhone->phone);
            }

            if(empty($checkPhone)){
                $checkPhone = $modelCustomers->newEmptyEntity();
                ...
                $checkPhone->birthday_date = 0;
                $checkPhone->birthday_month = 0;
                $checkPhone->birthday_year = 0;

                $checkPhone->created_at = time();
                $checkPhone->id_group = (int) @$dataSend['id_group'];
                ...
                if(!empty($infoMemberWeb)){
                    if($infoMemberWeb->type_member == 'member'){
                        $checkPhone->id_parent = (int) $infoMemberWeb->id;
                        $checkPhone->id_aff = 0;
                    }else{
                        $checkPhone->id_parent = 0;
                        $checkPhone->id_aff = (int) $infoMemberWeb->id;
                    }
                }

                $modelCustomers->save($checkPhone);
            }else{
                $checkPhone->full_name = $dataSend['name'];
