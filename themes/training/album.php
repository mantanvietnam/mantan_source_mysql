...
<main class="container mt-3 mt-lg-5">
    <div class="row g-4" id="main-post">
        <div class="col-lg-8">
            <h6 class="pb-4 mb-3 fst-italic border-bottom d-flex justify-content-between">
                <span><?php echo date('d/m/Y', $album->time_create);?></span>
                <span><?php echo $album->author;?></span>
            </h6>

            <article class="blog-post">
                <h1 class="blog-post-title text-primary-custom"><?php echo $album->title;?></h1>
                <!-- Content bài viết -->
                <div class="content-here">
                    <?php 
                        if(!empty($album->listImages)){
                            echo '  <ul id="lightSlider">';
                            foreach ($album->listImages as $key => $value) {
                                echo '  <li data-thumb="'.$value->image.'">
                                            <img src="'.$value->image.'" />
                                        </li>';
                            }  
                            echo '</ul>';
    ...
    <div class="row mb-2">
        <section class="chuong-trinh-dao-tao more-news">
            <div class="container">
                <h2 class="text-center text-secondary-custom">Album khác</h2>
                <div class="my-slider" loop="5">
                	<?php 
                	if(!empty($otherAlbums)){
                		foreach ($otherAlbums as $key => $value) {
